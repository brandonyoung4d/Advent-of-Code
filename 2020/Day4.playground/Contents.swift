...
class Passport {
    ...
    init(input: [String]) {
        ...
        for i in input {
            ...
            if components[0] == "byr" {
                let birthYear = components[1]
                guard let birthYearInt = Int(birthYear) else { continue }
                if birthYear.count == 4 && birthYearInt >= 1920 && birthYearInt <= 2002 {
                    self.byr = birthYear
                    print("Valid birth year = \(birthYear)")
                }
            }
            if components[0] == "iyr" {
                let issuerYear = components[1]
                guard let issuerYearInt = Int(issuerYear) else { continue }
                if issuerYear.count == 4 && issuerYearInt >= 2010 && issuerYearInt <= 2020 {
                    self.iyr = issuerYear
                    print("Valid issuer year = \(issuerYear)")
                }
            }
            if components[0] == "eyr" {
                let expirationYear = components[1]
                guard let expirationYearInt = Int(expirationYear) else { continue }
                if expirationYear.count == 4 && expirationYearInt >= 2020 && expirationYearInt <= 2030 {
                    self.eyr = expirationYear
                    print("Valid expiration year = \(expirationYear)")
                }
            }
            if components[0] == "hgt" {
                let height = components[1]
                if let heightUnitIndex = height.range(of: "cm") {
                    guard let heightInt = Int(height.prefix(upTo: heightUnitIndex.lowerBound)) else { continue }
                    if heightInt >= 150 && heightInt <= 193 {
                        self.hgt = height
                        print("Valid height = \(height)")
                    }
                } else if let heightUnitIndex = height.range(of: "in") {
                    guard let heightInt = Int(height.prefix(upTo: heightUnitIndex.lowerBound)) else { continue }
            ...
            if components[0] == "hcl" {
                var hairColor = components[1]
                if hairColor.removeFirst() != "#" { continue }
                if hairColor.count != 6 { continue }
                
                let validCharacters = CharacterSet.decimalDigits.union(CharacterSet.lowercaseLetters)
                let hairColorCharacterSet = CharacterSet(charactersIn: hairColor)
                
                if hairColorCharacterSet.isSubset(of: validCharacters) {
                    self.hcl = hairColor
                    print("Valid hair color = \(hairColor)")
                }
            }
            if components[0] == "ecl" {
                let eyeColor = components[1]
                let validColors = ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]
                if validColors.contains(eyeColor) {
                    self.ecl = eyeColor
                    print("Valid eye color = \(eyeColor)")
                }
            }
            if components[0] == "pid" {
                let passportID = components[1]
                if passportID.count != 9 { continue }
                if CharacterSet.decimalDigits.isSuperset(of: CharacterSet(charactersIn: passportID)) {
                    self.pid = passportID
                    print("Valid pasasport ID = \(passportID)")
                }
            }
            if components[0] == "cid" {
                self.cid = components[1]
            }
        }

        self.isValid = byr != nil && iyr != nil && eyr != nil && hgt != nil && hcl != nil && ecl != nil && pid != nil
    }
}

var passportStrings = input.components(separatedBy: "\n\n")
passportStrings = passportStrings.filter { $0.count > 0 }
print("Passport strings:")
print(passportStrings)

passportStrings = passportStrings.map { $0.replacingOccurrences(of: "\n", with: " ") }
print("\nPassport strings:")
print(passportStrings)
...
for passportString in passportStrings {
    let passportStringComponents = passportString.components(separatedBy: " ")
    var passport = Passport(input: passportStringComponents)
    passports.append(passport)
    if passport.isValid { validPassports += 1 }
}
