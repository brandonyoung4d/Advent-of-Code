...
print("Number of passwords = \(passwords.count)")

func findRequiredCharCount(_ string: String) -> (min: Int, max: Int) {
    let numbers = string.components(separatedBy: CharacterSet.decimalDigits.inverted)
    
    let minRequired = Int(numbers[0]) ?? 0
    let maxRequired = Int(numbers[1]) ?? 0
    
    for ignore in numbers[2...] {
        if !ignore.isEmpty {
            print("Error!")
        }
    }
...
func policy1() {
    ...
    for passwordInfo in passwords {
        let minMaxRequired = findRequiredCharCount(passwordInfo)
        
        if let requiredCharIndex = passwordInfo.firstIndex(where: { $0.isLetter }) {
            let requiredChar = passwordInfo[requiredCharIndex]
            let passwordStartIndex = passwordInfo.index(requiredCharIndex, offsetBy: 3)
            let password = passwordInfo[passwordStartIndex...]
            let requiredCharCount = password.filter({ $0 == requiredChar }).count
            
            if requiredCharCount >= minMaxRequired.min && requiredCharCount <= minMaxRequired.max {
    ...
    print("Found \(correctPasswordCount) passwords")
}
...
func policy2() {
    ...
    for passwordInfo in passwords {
        ...
        if let requiredCharIndex = passwordInfo.firstIndex(where: { $0.isLetter }) {
            ...
            let requiredCharIndex1 = password.index(password.startIndex, offsetBy: minMaxRequired.min - 1)
            let requiredCharIndex2 = password.index(password.startIndex, offsetBy: minMaxRequired.max - 1)
            let requiredChar1 = password[requiredCharIndex1]
            let requiredChar2 = password[requiredCharIndex2]
            
            if requiredChar1 == requiredChar && requiredChar2 != requiredChar {
                correctPasswordCount += 1
            } else if requiredChar2 == requiredChar && requiredChar1 != requiredChar {
                correctPasswordCount += 1
            }
        } else {
            print("Error2!")
